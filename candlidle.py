...
class Player(object):
    ...
    def move(self):
        ...
        if self.position + roll <= 99:
            ...
            print("you have to roll", str(99 - self.position), "to land on 100")
...
# functions
def ask_num(question, num1, num2):
   while True:
       print(question)
       userinput = input()
       if userinput.isdigit() and int(userinput) >= num1 and int(userinput) <= num2:
           return userinput
       else:
...
def main():
    for i in range(100):
        ...
        p3p.append(EMPTY)
        p4p.append(EMPTY)
    num_players = int(ask_num("how many people will be playing", 2, 4))    
    players = []
    turn = 0
    winner = None
